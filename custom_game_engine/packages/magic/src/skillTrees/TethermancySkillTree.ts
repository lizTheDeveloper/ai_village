/**
 * TethermancySkillTree - Skill tree for the Tethermancy paradigm
 *
 * Key mechanics:
 * - Attunement (mental focus/belief) - the core skill
 * - Split focus - hold multiple bindings simultaneously
 * - Link types - different ways to connect objects
 * - Drift reduction - energy loss in transfers
...
// ============================================================================
// Foundation Nodes - Attunement and Basic Tethermancy
// ============================================================================

/** Entry node - basic mental focus */
const BASIC_ATTUNEMENT_NODE = createSkillNode(
  'basic-attunement',
  'Basic Attunement',
  PARADIGM_ID,
  'foundation',
  ...
  {
    description: 'Develop the basic mental focus needed for tethermancy',
    lore: `Attunement is the cornerstone of tethermancy. It is the deep, true belief that something
...
/** Heart of stone - emotional control for stable attunement */
const HEART_OF_STONE_NODE = createSkillNode(
  ...
  100,
  [
    createSkillEffect('attunement_strength', 2, {
      description: 'Attunement unaffected by emotional state',
    }),
  ...
  {
    description: 'Learn to separate emotions from your attunement',
    lore: 'A tethermancer with a heart of stone can maintain bindings even under extreme duress.',
    prerequisites: ['basic-attunement'],
  }
);
...
const DUAL_BINDING_NODE = createSkillNode(
  ...
  200,
  [
    createSkillEffect('attunement_split', 1, {
...
const QUAD_BINDING_NODE = createSkillNode(
  ...
  ],
  {
    ...
    unlockConditions: [
      createUnlockCondition(
        'skill_level',
        { skillId: 'tethermancy', skillLevel: 4 },
        'Requires near-mastery of tethermancy'
      ),
    ],
    conditionMode: 'all',
  }
...
// ============================================================================
// Drift and Efficiency Nodes
// ============================================================================

/** Drift reduction */
const DRIFT_CONTROL_NODE = createSkillNode(
  'drift-control',
  'Drift Control',
  PARADIGM_ID,
  'efficiency',
  ...
  150,
  [
    createSkillEffect('drift_reduction', 5, {
      perLevelValue: 3,
  ...
  ],
  {
    ...
    lore: 'Drift is the bane of all tethermancers - energy bleeds off during transfer, limiting what can be accomplished.',
...
const PERFECT_LINK_NODE = createSkillNode(
  ...
  400,
  [
    createSkillEffect('drift_reduction', 20, {
      description: 'Near-zero drift on identical links',
    }),
  ],
  {
    description: 'Create nearly perfect sympathetic links with minimal drift',
    prerequisites: ['drift-control', 'link-identical'],
    unlockConditions: [
      createUnlockCondition(
        'magic_proficiency',
...
/** Attunement defense */
const ATTUNEMENT_DEFENSE_NODE = createSkillNode(
  'attunement-defense',
  'Iron Will',
  PARADIGM_ID,
  'technique',
  ...
  [
    createSkillEffect('unlock_ability', 1, {
      target: { abilityId: 'attunement_shield' },
      description: 'Can resist hostile bindings with pure will',
    }),
  ],
  {
    description: 'Use your attunement to defend against sympathetic attacks',
...
/** Tether Effigy creation */
const TETHER_EFFIGY_NODE = createSkillNode(
  'tether_effigy',
  'Tether Effigy Binding',
  PARADIGM_ID,
  'mastery',
  4,
  450,
  [
    createSkillEffect('unlock_ability', 1, {
      target: { abilityId: 'create_tether_effigy' },
      description: 'Can create tether_effigys for remote sympathetic work',
    }),
  ],
  {
    description: 'Learn to create tether_effigys - sympathetic dolls for remote binding',
    lore: 'A tether_effigy is a doll linked to a person through their blood, hair, or saliva. What is done to the tether_effigy affects the person.',
    prerequisites: ['body-binding', 'link-once_together'],
    unlockConditions: [
      createUnlockCondition(
        'skill_level',
        { skillId: 'crafting', skillLevel: 2 },
        'Must have crafting skill to create tether_effigys'
      ),
    ],
    conditionMode: 'all',
  }
...
/** Shieldweave binding - work without physical link */
const SHIELDWEAVE_BINDING_NODE = createSkillNode(
  'shieldweave-binding',
  'Shieldweave Tethermancy',
  PARADIGM_ID,
  'mastery',
  5,
  600,
  [
    createSkillEffect('unlock_ability', 1, {
      target: { abilityId: 'shieldweave_link' },
      description: 'Can create bindings through pure attunement without physical links',
    }),
  ],
  {
    description: 'The legendary skill of creating bindings through attunement alone',
    lore: 'Only the greatest tethermancers can create bindings without any physical connection - through sheer force of will.',
...
const XP_SOURCES: MagicXPSource[] = [
  ...
  {
    eventType: 'drift_minimized',
    xpAmount: 15,
    description: 'Complete a transfer with minimal drift',
  },
  {
    eventType: 'tethermancy_study',
    xpAmount: 5,
...
const ALL_NODES: MagicSkillNode[] = [
  ...
  // Defense
  LINK_BREAKING_NODE,
  ATTUNEMENT_DEFENSE_NODE,

  // Advanced
  BODY_BINDING_NODE,
  TETHER_EFFIGY_NODE,
  SHIELDWEAVE_BINDING_NODE,
];

/**
 * The Tethermancy skill tree.
 * Learned through study - no innate requirement.
 */
export const TETHERMANCY_SKILL_TREE: MagicSkillTree = {
  id: 'tethermancy-tree',
  paradigmId: PARADIGM_ID,
  name: 'The Art of Tethermancy',
  description: 'Create links between objects based on similarity and shared history, allowing energy to flow between them.',
  lore: `Tethermancy is based on two principles: Like affects like, and once together, always together.
Master your attunement - your deep belief - and you can create links between objects that share
properties or history. Through these links, energy flows: heat, motion, force. But beware
drift - energy is always lost in the transfer, and working too hard can burn you out.`,
  nodes: ALL_NODES,
  entryNodes: ['basic-attunement'],
...
/**
 * Calculate drift percentage based on nodes.
 */
export function calculateDrift(
  baseDrift: number,
  driftControlLevel: number,
  hasPerfectLink: boolean,
  linkType: string
): number {
  let drift = baseDrift;

  // Drift control reduces by 5% + 3% per level
  drift -= driftControlLevel * 0.03 + (driftControlLevel > 0 ? 0.05 : 0);
  ...
  return Math.max(0.01, drift); // Minimum 1% drift
}
