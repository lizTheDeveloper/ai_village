...
export class ExpressionEvaluator {
  ...
  private fn_has_status(args: Expression[], context: ExpressionContext, depth: number): boolean {
    // Allow 1 or 2 arguments: has_status(statusName) checks target, has_status(entity, statusName) checks specific entity
    if (args.length < 1 || args.length > 2) {
      throw new Error('has_status requires 1 or 2 arguments (statusName) or (entity, statusName)');
    }
    ...
    if (args.length === 1) {
      // Single argument: check status on target
      entityPath = 'target';
      statusName = args[0] as string;
    } else {
      // Two arguments: entity path and status name
      entityPath = args[0] as string;
      statusName = args[1] as string;
    }

    if (typeof entityPath !== 'string') {
      throw new Error('has_status entity argument must be a variable path');
    }
    if (typeof statusName !== 'string') {
      throw new Error('has_status status argument must be a string');
    }
