...
/**
 * Get context for a specific skill at a given level.
 * Returns null for untrained skills (level 0).
 */
export function getSkillContext(skillId: SkillId, level: SkillLevel): string | null {
  return SKILL_CONTEXTS[skillId]?.[level] ?? null;
}

/**
 * Get formatted skill context section for LLM prompt.
 * Combines relevant skill contexts based on agent's skill levels.
 */
export function buildSkillContextSection(
  skills: Record<SkillId, SkillLevel>
): string {
  const contexts: string[] = [];

  // Get contexts for skills level 1 and above
  for (const [skillId, level] of Object.entries(skills) as [SkillId, SkillLevel][]) {
    if (level >= 1) {
      const context = getSkillContext(skillId, level);
      if (context) {
        const skillName = skillId.replace(/_/g, ' ');
        const levelName = getLevelName(level);
        contexts.push(`[${skillName.toUpperCase()} - ${levelName}]\n${context}`);
      }
    }
  }
 ...
 */
function getLevelName(level: SkillLevel): string {
  switch (level) {
...
/**
 * Get a brief skills summary for compact display.
 */
export function getSkillsSummary(skills: Record<SkillId, SkillLevel>): string {
  const trainedSkills: string[] = [];

  for (const [skillId, level] of Object.entries(skills) as [SkillId, SkillLevel][]) {
    if (level >= 1) {
      const skillName = skillId.replace(/_/g, ' ');
      trainedSkills.push(`${skillName}: ${getLevelName(level)}`);
    }
  }

  if (trainedSkills.length === 0) {
    return 'Skills: None trained yet';
  }

  return `Skills: ${trainedSkills.join(', ')}`;
}
