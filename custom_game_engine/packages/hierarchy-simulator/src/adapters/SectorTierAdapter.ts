      ...
      for (let j = i + 1; j < ftlSystems.length; j++) {
        ...
        if (Math.random() < gateChance) {
          ...
          abstractSector.infrastructure.wormholeGates.push({
            ...
            destinationSystem: ftlSystems[j].id,
            distance,
            travelTime,
    ...
    for (let i = 0; i < networkCount; i++) {
      ...
      while (connectedSystems.length < Math.min(systemCount, spacefaringSystems.length)) {
        ...
        if (!selectedIndices.has(idx)) {
          ...
          connectedSystems.push(spacefaringSystems[idx].id);
        }
      }
    ...
    if (abstractSector.politicalEntities.length > 1) {
      ...
      for (let i = 0; i < abstractSector.politicalEntities.length; i++) {
        for (let j = i + 1; j < abstractSector.politicalEntities.length; j++) {
          ...
          const stance = stances[Math.min(stanceRoll, stances.length - 1)];

          abstractSector.politicalEntities[i].diplomaticStance.set(abstractSector.politicalEntities[j].id, stance);
          abstractSector.politicalEntities[j].diplomaticStance.set(abstractSector.politicalEntities[i].id, stance);
