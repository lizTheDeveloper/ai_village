...
import type { World, EventBus, GameEventMap } from '@ai-village/core';
...
import type { RadialMenuItem, RadialMenuConfig, MenuState, VisualState, ItemRenderState, MenuStackEntry, ContextAction } from './context-menu/types.js';
...
export class ContextMenuManager {
  ...
  constructor(world: World, eventBus: EventBus, camera: Camera, canvas: HTMLCanvasElement) {
    if (!world) {
   ...
   */
  getVisibleItems(): RadialMenuItem[] {
   ...
   */
  getItemRenderState(itemId: string): ItemRenderState {
  ...
  handleShortcut(key: string): void {
    ...
    const screenPos = (this.camera as any).worldToScreenSimple(this.hoverWorldX, this.hoverWorldY);
   ...
   */
  setHoverPosition(worldX: number, worldY: number): void {
   ...
   */
  getSubmenuItems(parentId: string): RadialMenuItem[] | null {
   ...
   */
  selectAction(itemId: string): void {
    this.eventBus.emit({
      type: 'ui:contextmenu:action_selected',
      source: 'world',
  ...
  render(ctx?: CanvasRenderingContext2D): void {
    ...
    if (this.state.isAnimating) {
      ...
      renderer.render(this.currentItems, this.state.position.x, this.state.position.y);
    }
  }
  ...
  private actionsToMenuItems(actions: ContextAction[], context: MenuContext): RadialMenuItem[] {
    return actions.map((action, index) => {
      ...
      if (action.id === 'enter') {
        ...
        const buildingComp = building ? (building.getComponent('building') as any) : undefined;
        isEnabled = buildingComp?.canEnter === true && buildingComp?.locked !== true;
      } else if (action.id === 'repair') {
        const building = context.getTargetEntity(this.world);
        const buildingComp = building ? (building.getComponent('building') as any) : undefined;
        isEnabled =
          buildingComp !== undefined &&
          buildingComp.health !== undefined &&
          buildingComp.health < 1.0;
      } else if (action.id === 'harvest') {
        const resource = context.getTargetEntity(this.world);
        const harvestable = resource ? (resource.getComponent('harvestable') as any) : undefined;
        isEnabled =
          harvestable !== undefined && harvestable.amount !== undefined && harvestable.amount > 0;
  ...
  private setupEventListeners(): void {
    ...
    const confirmHandler = (event: any) => {
      ...
      this.registry.execute(data.actionId, data.context);
    };
  ...
  cleanupEventListeners(): void {
    for (const { event, handler } of this.eventListeners) {
      this.eventBus.off(event as keyof GameEventMap, handler as (event: GameEventMap[keyof GameEventMap]) => void);
    }
    this.eventListeners = [];
  }
