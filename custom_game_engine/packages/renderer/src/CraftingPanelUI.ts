...
import { RecipeDetailsSection } from './RecipeDetailsSection.js';
...
export class CraftingPanelUI {
  ...
  public readonly recipeDetailsSection: RecipeDetailsSection;
  ...
  constructor(world: World, canvas: HTMLCanvasElement) {
    ...
    this.recipeDetailsSection = new RecipeDetailsSection(
      world,
      detailsX,
      detailsY,
      detailsWidth,
    ...
    // Connect craft callbacks
    this.recipeDetailsSection.onCraftNow((recipeId: string, quantity: number) => {
      if (this._onCraftNowCallback) {
        this._onCraftNowCallback(recipeId, quantity);
      }
    });

    this.recipeDetailsSection.onAddToQueue((recipeId: string, quantity: number) => {
      if (this._onAddToQueueCallback) {
        this._onAddToQueueCallback(recipeId, quantity);
      }
    });
  ...
  setActiveAgent(agentId: EntityId): void {
    ...
    this.recipeDetailsSection.setAgentId(agentId);
