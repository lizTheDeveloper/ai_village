...
import positionUpdateShaderSource from './shaders/position-update.wgsl?raw';
...
export class GPUPositionIntegrator {
  ...
  async updatePositions(
    ...
    this.device.queue.writeBuffer(this.velXBuffer!, 0, velocitiesX, 0, count);
    this.device.queue.writeBuffer(this.velYBuffer!, 0, velocitiesY, 0, count);
    this.device.queue.writeBuffer(this.speedMultiplierBuffer!, 0, speedMultipliers, 0, count);
