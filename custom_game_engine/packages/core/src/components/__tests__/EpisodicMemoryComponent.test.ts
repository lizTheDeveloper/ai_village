...
import { EpisodicMemoryComponent } from '../EpisodicMemoryComponent';
...
describe('EpisodicMemoryComponent', () => {
  ...
  describe('memory immutability', () => {
    it('should prevent editing memory content', () => {
      ...
      expect(() => {
        (formed as any).summary = 'Changed description';
      }).toThrow();
    ...
    it('should prevent deletion of memories', () => {
      ...
      expect((memory as any).deleteMemory).toBeUndefined();
      expect((memory as any).removeMemory).toBeUndefined();
    });
