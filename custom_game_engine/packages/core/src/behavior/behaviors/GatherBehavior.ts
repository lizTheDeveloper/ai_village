...
export class GatherBehavior extends BaseBehavior {
  ...
  private findGatherTarget(
    ...
    if (resourceTarget && plantTarget) {
      ...
      // Prefer seeds if explicitly seeking, plant is much closer, or have enough of preferred resource
      if (isSeekingSeeds || plantTarget.distance * 2 < resourceTarget.distance || hasEnoughPreferred) {
  ...
  private gatherSeeds(
    ...
    try {
      ...
      console.log(`[GatherBehavior] Agent ${entity.id} gathered ${result.amountAdded} ${seedItemId} from ${plantEntity.id}`);
      ...
      // Inventory full
      console.log(`[GatherBehavior] Agent ${entity.id} inventory full: ${(error as Error).message}`);
      const agent = entity.getComponent<AgentComponent>('agent')!;
      this.handleInventoryFull(entity, world, agent);
    }
  }
