...
export async function compress(data: string): Promise<string> {
  ...
  if (typeof CompressionStream !== 'undefined') {
    ...
    const compressedStream = (stream as any).pipeThrough(new CompressionStream('gzip'));
...
export async function decompress(compressedData: string): Promise<string> {
  ...
  if (typeof DecompressionStream !== 'undefined') {
    ...
    const decompressedStream = (stream as any).pipeThrough(new DecompressionStream('gzip'));
