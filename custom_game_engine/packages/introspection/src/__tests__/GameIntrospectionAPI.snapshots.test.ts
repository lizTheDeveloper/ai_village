...
class MockGameIntrospectionAPI {
  private world: World;
  private snapshots: Map<SnapshotId, any>;
  ...
  async createSnapshot(
    ...
    for (const entityId of entityIds) {
      const entity = this.world.getEntity?.(entityId) as any;
      if (!entity) {
      ...
      for (const compType of ['test_agent', 'test_needs', 'test_position']) {
        if (entity.hasComponent(compType)) {
          const comp = entity.getComponent(compType);
    ...
    for (const [entityId, entityState] of snapshot.entities.entries()) {
      ...
      for (const [componentType, componentData] of Object.entries(
        ...
        if (entity.hasComponent(componentType)) {
          ...
          const current = entity.getComponent(componentType);
          const restored = { ...current, ...componentData };
          entity.updateComponent(componentType, () => restored);
        }
      }
...
describe('GameIntrospectionAPI - Snapshots & Time Travel', () => {
  ...
  beforeEach(() => {
    ...
    (entity1 as any).addComponent(TestAgentSchema.createDefault());
    (entity1 as any).addComponent(TestNeedsSchema.createDefault());
    (entity1 as any).addComponent(TestPositionSchema.createDefault());

    entity2 = createMockEntity('entity-2');
    (entity2 as any).addComponent({
      type: 'test_agent',
      version: 1,
      name: 'Alice',
      level: 5,
  ...
  describe('createSnapshot()', () => {
    ...
    it('should serialize all components correctly', async () => {
      ...
      const entity = world.getEntity?.('entity-2') as any;
  ...
  describe('restoreSnapshot()', () => {
    it('should restore single entity from snapshot', async () => {
      ...
      const entityBefore = world.getEntity?.('entity-1') as any;
      ...
      const entityAfter = world.getEntity?.('entity-1') as any;
    ...
    it('should restore multiple entities from snapshot', async () => {
      ...
      const entity1After = world.getEntity?.('entity-1') as any;
      const entity2After = world.getEntity?.('entity-2') as any;
    ...
    it('should restore component values correctly', async () => {
      ...
      const entity = world.getEntity?.('entity-2') as any;
      entity.updateComponent('test_agent', (c: any) => ({ ...c, level: 10 }));
    ...
    it('should restore all components for entity', async () => {
      ...
      const entity = world.getEntity?.('entity-1') as any;
      entity.updateComponent('test_agent', (c: any) => ({ ...c, name: 'Bob', level: 20 }));
      entity.updateComponent('test_needs', (c: any) => ({
        ...c,
        hunger: 0.1,
        thirst: 0.2,
      ...
      entity.updateComponent('test_position', (c: any) => ({ ...c, x: 100, y: 200 }));
  ...
  describe('Integration: Full Workflow', () => {
    it('should support full snapshot → mutate → restore workflow', async () => {
      ...
      const entity1 = world.getEntity?.('entity-1') as any;
      const entity2 = world.getEntity?.('entity-2') as any;
