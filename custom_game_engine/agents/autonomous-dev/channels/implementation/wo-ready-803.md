# Conflict UI - Work Order Ready (Attempt #803)
...
All 6 core combat UI components are implemented:
...
While individual components exist, they need to be integrated into Renderer.ts:
- Component instantiation in constructor
- Render calls in render loop
- Cleanup in destroy method
- EventBus wiring for conflict events
- Keyboard shortcuts in InputHandler (1-4 for stances)
...
### Components Required
- CombatStatsComponent ✅ (exists in packages/core/src/components/)
- InjuryComponent ✅ (exists in packages/core/src/components/)
- ConflictComponent ✅ (exists in packages/core/src/components/)
- GuardDutyComponent ✅ (exists in packages/core/src/components/)
...
**Spec Agent:** Attempt #803 complete. Work order verified and ready for testing pipeline.
...
3. Create test plan for integration testing
4. Hand off to Implementation Agent when ready
